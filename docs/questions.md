...
## Як реалізовувати Actions?

> Усе висновки написані нище зроблені на основі [5-го розділу дисертації Роя Філдинга](https://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm),
> і запису [REST APIs must be hypertext-driven](https://roy.gbiv.com/untangled/2008/rest-apis-must-be-hypertext-driven) 
> в його блозі.

Для деякого розуміння цієї дилеми потрібно спочатку зрозуміти реальне значення підходу HATEOAS. HATEOAS про керування
станом серверу через переходи, які сервер сам надає клієнту. Гарним прикладом є те як реалізований цей підхід в 
html. Розглянемо основні положення підходу HATEOAS та те як вони реалізовуються в HTML. 
...
початковий URI буде повиртати ресурс "Головна сторінка", що може мати свій окремий медіа тип. 
...
> бути використаний при відправці запиту на цей URI. Інформація про http метод може бути вказана в описі медіа типу.
> Наприклад html форми по-замовчуванню відправляються через GET метод, або запити на URI який вказано в атрибуті href 
> тегу зображення також відправляються з GET методом.

- Клієнт не повинен знати завчасно про можливі дії, які можна виконувати над ресурсом. Інформація про усі дії повинна 
бути отримана клієнтом від серверу. Хоча й множина можливих дій може бути визначена в опису медіа типу ресурсу, в цьому 
випадку достатньо знання, що сервер підтримує необхідний медіа тип. **Важливо: http методи не визначають множину дій, 
які можуть бути виконані надо ресурсом. Елементи медіа типу визначають множину можливих дій.** 
 
    HTTP методи визначають: 
  - Чи операція безпечна або ідемпотентна
...
множину дій над ресурсом.*
    ...
    *Тобто отримуючи елемент колекції, ми також можемо отримати:*
...
Openapi маніфести ломають одну з головних концепцій REST API: Uniform Interface. Адже клієнт завчасно не повинен знати
ні те як будувати URI, ні те які методи доступні на даний URI, ні те що повернеться. Єдине знання доступне клієнт - це 
початковий URL та множина можливих медіа типів, що використовуються в цьому REST API.
...
Таким чином для чистого REST використання маніфестів можна використовувати лише для генерації серверного коду, клієнт
же повинен орієнтуватись на початковий URL та множину медіа типів.

Але я не бачу нічого страшного в використані openapi манфіестів для генерації клієнтського коду в звичному нам стилі.
Умовно ми можемо сказати, що openapi маніфест - це окремий медіа тип, який повертається клієнту з початкового url, та
містить інформацію про усі подальші можливі дії з API. Звістно це лише умовно, робити насправді початковий url з якого
повертається openapi маніфест не обов'язково (хоча й на справді може бути інколи корисним).

### Висновки

- HTTP методи - це не CRUD, та вони взагалі не визначають сенс операції (хоча можуть накладати певні обмеження які роблять
їх більш або менш корисними для певних типів операцій). 
...
## Як повинна відбуватись конфігурація роутера?
...
## Як спростити пошук реалізації маніфеста?
...
Генератор складається з декількох рівні, що розміщені в заданому порядку, та кожен є абстракцією над попереднім рівнем.
Кожен рівень відповідає своїм функціям, та за потреби може використовувати, або не використовувати рівні, що знаходяться
нижче. Та не може використовувати рівні, що знаходяться вище.

Ідея рівні близька до [мережевої моделі OSI](https://uk.wikipedia.org/wiki/%D0%9C%D0%B5%D1%80%D0%B5%D0%B6%D0%B5%D0%B2%D0%B0_%D0%BC%D0%BE%D0%B4%D0%B5%D0%BB%D1%8C_OSI),
та в деякому сенсі рівні генератора можна розглядати як продовження цієї моделі (тобто перший рівень генератора, це
сьомий рівень OSI). Але відміності від OSI моделі існують, тому таку аналогію потрібно проводити з певною обережністю.
