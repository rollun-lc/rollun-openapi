...
* [1. Api специфікація](#1-api-)
  ...
  * [Чи можуть різні сервери реалізовувати один маніфест віддаючи різні відповіді на однаковий запит? ❌ ❌ ❌](#---------------)
      * [Маніфест для керування пристроєм, якого фізично існує декілька копій](#--------)
      * [Сервіси, що слідують єдиному інтерфейсу](#----)
  * [Як працювати з маніфестами при розробці ✅ ❌ ❌](#--------)
    * [Чи вказувати локальні сервери в маніфесті?](#-----)
* [2. Openapi генератор](#2-openapi-)
  * [2.1 Вибір сервера ❌ ❌ ❌](#21-----)
  * [2.2 Шлях до згенерованного коду ✅ ❌ ❌](#22-------)
  * [2.3 Конфігурація ❌ ❌ ❌](#23----)
  * [2.4 Авторизація ❌ ❌ ❌](#24----)
  ...
  * [2.5 Підтримка oneOf ❌ ❌ ❌](#25--oneof---)
  * [2.6 Організація схем, що використовуються у декількох маніфестах ❌ ❌ ❌](#26----------)
  * [2.7 Конфігурація роутера ❌ ❌ ❌](#27-----)
  * [2.8 Як спростити пошук реалізації маніфеста? ❌ ❌ ❌](#28--------)
  * [2.9 Вимоги до генератора ❌ ❌ ❌](#29------)
    * [Архітектура](#)
      * [Http рівень](#http-)
      * [Openapi рівень](#openapi-)
      * [Конвертація: `Psr\Request` -> `Openapi\Request`](#--psrrequest----openapirequest)
      * [Rollun layer](#rollun-layer)
      * [Конвертація `OpenapiRequest` -> `Command`](#-openapirequest----command)
      * [Конвертація `Result` -> `OpenapiResponse`](#-result----openapiresponse)
...
В [документації](https://swagger.io/docs/specification/api-host-and-base-path/) openapi немає ніяких обмежень
на те як повинні поводити себе сервера, якщо їх вказанно декілька. І судячи з їх прикладів, сервери не повинні
дублювати один одного. Наприклад вони пишуть, що можна вказати декілька серверів: продакшн, staging, sandbox. Прикладів
з тим щоб сервера дублювали один одного немає. 
...
можуть. Існує багато прикладів, де за одним інтерфейсом скрито багато реалізацій. Наприклад вбудований в php 
інтерфейс Stringable. Кожен об'єкт, що реалізує цей інтерфейс може повертати абсолютно різні данні.

Якщо потрібен приклад для http api, то ми можемо створити інтерфейс HealthChecker, який буде мати ендпоінт /ping. Кожен
сервер буде відповідати різними даними в залежності від того активний він чи ні

Більше прикладів:

#### Маніфест для керування пристроєм, якого фізично існує декілька копій

Це може api по керуванню чим завгодно: чайником, принтером, розумною лампочкою і т.п.

Для прикладу візьмемо дві відеокамери однакової моделі, кожна підключена до інтернету і має власний ip: перша - 
123.123.1.1, друга - 123.123.1.2. Фізично камери можуть знаходитись де завгодно: хоч в одній кімнаті хоч на різних 
континентах. І ми хочемо написати api, що дозволяє зчитувати та змінювати налаштування кожної з камер. Тоді в маніфесті
для цього api буде вказано два сервери.

```yaml
servers:
  - url: http://123.123.1.1
    description: video camera 1
  - url: http://123.123.1.2
    description: video camera 2
```

Однаковий запит на зчитування налаштувань може повернути різний результат в залежності від того, до якої камери ми його 
відправили.

#### Сервіси, що слідують єдиному інтерфейсу

Візьмемо приклад з нашої фірми. У нас є декілька різних постачальників та для кожного написано власний сервер.
Тоді, якщо ми можемо створити єдиний інтерфейс, якому буде здатний слідувати кожен с сервісів, то в одному маніфесті
буде вказано по серверу для кожно з постачальників, та запити на ці сервери будуть повертати різні данні. 

Як показує практика ми можемо створити такі універсальні інтерфейси, та вже маємо їх: shippingWays, SuppliersOrders.

*Що робити якщо ми робимо api для автозакупки та хочемо зробити enum зі складами, адже для кожного постачальника цей
enum різний?*

Подумати навіщо нам це потрібно. Якщо на клієнті нема потреби писати різний код для різних значень цього енаму, то
можливо він не потрібен. Наприклад, при автозакупці ми беремо назву складу з поля в угоді мегаплану, та алгоритм ніяк
не змінюється в залежності від того що за значення там знаходиться. 

Значення в це поле вноситься с запиту до shippingWays (howToBuy), тож не валідне значення програмно потрапити туди не 
може. Якщо ж нам хочется зробити так, щоб людина вручну не могла написати туди не валідне значення, то ми можемо створити
ендпоінт який буде повертати для кожного постачальника список валідних назв складів, та перед зберіганням перевіряти,
що значення поля є допустимим.

Якщо ж нам реально потрібно під кожен склад писати різну логіку на клієнті, то скоріше за все це буде логіка
специфічна для кожного з постачальників і тоді логічно написати окремі НЕ універсальні маніфести, де в кожному буде
по одному серверу.
...
## 2.6 Організація схем, що використовуються у декількох маніфестах ❌ ❌ ❌
...
та умовно рівні генератора можна розглядати як продовження цієї моделі: перший рівень генератора, це сьомий 
...
> 
> При цьому при реалізації генератор реальні назви будуть відрізнятись. Наприклад `Psr\Request` насправді 
...
Конвертація `Openapi\Response` -> `Psr\Request` в цілому зеркальна. Та навіть простіша, оскільки медіа тип вказанно
в заголовках. Потрібно лише провести валідацію, що об'єкт відповідає своєму медіа типу.

Головне, що потрібно передбачити:
- Десеріалізація даних в об'єкти з підтримкою поліморфізму (oneOf (дискримінатор), anyOf, allOf)
- Підтримка різних схем при різних медіа типах
- Підтримка різних кодувань
- Валідація даних згідно з маніфестом
- Валідація даних згідно з медіа типом
...
3. Якщо цей заголовок використовується в багатьох сервісах, то можна додати його до нашої специфікаці і відповідно
   випустити нову версію генератора, який розширює класс `Command`

#### Конвертація `Result` -> `OpenapiResponse`
