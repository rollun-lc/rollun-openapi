...
Існують зміни, що порушують зворотну сумісність лише на одній стороні (клієнті або сервері) та не порушують на іншій.
Цю проблему можна вирішувати різними шляхами. 

Наприклад створити окреме версіонування для клієнта і для сервера. Для цього доведеться або створювати окремі маніфести 
для клієнта і сервера, або вказувати версію в кастомному полі (здається так можна). Обидва цих варіанти на мою думку 
будуть приводити до складності в менеджменті маніфестів та плутати програмістів.

Створювання окремих маніфестів буде приводити до складності узгодження їх між собою.

Прописування окремих версій в одному маніфесті також може приводити до складностей. Наприклад візьмемо зменшення 
діапазону enum значень. Якщо зробити два різних маніфести, то в маніфесті для сервера можна видалити значення, що не 
буде використовуватись, а для клієнта залишити його. Але якщо версії будуть вказуватись в одному маніфесті, то не 
зовсім зрозуміло, що робити. Звісно можна залишити це значення написавши в description, що воно deprecated, як і буде
пропонуватись пізніше, але тоді відпадає потреба в окремому версіонуванні.

Тож я пропоную використовувати одну версію і для клієнта і для сервера. Зміна буде вважатись такою, що порушує 
зворотну сумісність тоді, коли вона це робить хоча б для однієї зі сторін (тобто або для клієнта і для сервера).
Аналогічно зміна буде вважатись такою, що не порушує зворотну сумісність тоді, коли вона не робить цього і для клієнта
і для сервера.
...
описані в пунктах нижче):
- Змінна типу поля
- Зміна значення (сенсу) поля, навіть якщо його тип не змінився
- Додання обмежень до логіки валідації
- Зменшення діапазону enum, як вхідного параметру<sup>[1](#135-виноски)</sup>
- Видалення ендпоінту
- Додання обов'язкового поля
- Зміна поля з необов'язкового на обов'язкове

### 1.3.3 Коли змінювати мінорну версію

При змінах, що не порушують зворотну сумісність:
- Додання необов'язкового поля до вихідних параметрів<sup>[2](#135-виноски)</sup> 
  > При умові, що клієнти ігнорують усі невідомі їм поля
- Додання необов'язкового поля до вхідних параметрів<sup>[1](#135-виноски)</sup>
- Зменшення діапазону enum як вихідного параметру. 
  > Але важливо при цьому не видаляти це значення з маніфесту, оскільки порушиться валідація на клієнтській стороні, а 
  > лише помітити значення як deprecated. 
  > 
  ...
  >   description: |
  >     pending - ...
  >     in_progress - deprecated. Use `pending` instead. 
  >     fulfilled - ...
  >     rejected - ...
  >   enum:
  >     - pending
...
- Додання документації, прикладів в маніфест

> Якщо виправлення помилки приводить до порушення зворотної сумісності, то, при **повній впевненості**, що це не 
> приведе до ще більших помилок, можна не змінювати мажорну версію.

### 1.3.4 Виноски

1. Вхідні параметри - це тіло запиту, query параметри та заголовки (у запиті)
2. Вихідні параметри - це тіло відповіді, заголовки (у відповіді)
...
Openapi маніфестах можна [розширяти](https://swagger.io/docs/specification/openapi-extensions/) вказуючи в них свої 
...
Пагінація яка використовується в наших датасторах, факточно вказується номер наступного після останнього отриманого 
елементу. Зазвичай зміщення вказується не явно, а через номер сторінки та кількість елементів на сторінці, тоді 
