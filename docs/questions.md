...
Вибір сервера, на який буде відправлятись запит буде здійснюватись за допомогою декораторів для `Operation` 
...
![Request sender](./img/arch/host-selection.jpg)
...
Сам декоратор буде приймати цей enum у якості сервера, та буде отримувати host за допомогою методу `Server::url()`.
