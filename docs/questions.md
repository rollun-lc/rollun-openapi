...
Зміна буде вважатись такою, що порушує зворотну сумісність тоді, коли вона це робить хоча б для однієї зі сторін (тобто 
або для клієнта або для сервера). Аналогічно зміна буде вважатись такою, що не порушує зворотну сумісність тоді, коли 
вона не робить цього і для клієнта і для сервера.
...
- Замість зміни сенсу, або типу поля можна стоврити нове поле, та підтримувати, до наступної 
мажорної версії маніфесту, обидва поля. 
- При доданні нового ендпоінту варто задуматись, чи не заслуговує цей ендпоінт на окремий маніфест. Тримати усі 
ендпоінти в одному маніфесті погано по тим самим міркуванням чому існує SOLID принцип про розділення інтерфейсів. 

### 1.3.3 Коли змінювати мінорну версію

При змінах, що не порушують зворотну сумісність:
- Додання необов'язкового поля до вихідних параметрів<sup>[1](#135-виноски)</sup> 
  > При умові, що клієнти ігнорують усі невідомі їм поля
- Додання необов'язкового поля до вхідних параметрів<sup>[2](#135-виноски)</sup>
- Помітити enum значення, що використовується *лише у вихідних параметрах*, як deprecated.
  ...
  >     - rejected
  >```
- Помітити поле/ендпоінт як depracated
...
1. Вихідні параметри - це тіло відповіді, заголовки (у відповіді)
2. Вхідні параметри - це тіло запиту, query параметри та заголовки (у запиті)
