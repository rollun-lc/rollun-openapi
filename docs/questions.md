...
Усі поля ресурсу в маніфесті повинні бути об'явленні як optional. При цьому запит повинен мати select за замовчуванням.
Якщо запит не підтримує select (наприклад за ненадобності), то повинні повертатись усі поля.

Єдине що мене хвилює це чи можлива ситуація коли у відповіді потрібно явно відрізняти поле зі значенням null і 
відсутність поля взагалі. Що тоді повинно повернутись якщо select запитує поле, яке відсутнє. 

Я поки не можу придумати приклад такої ситуації. А у випадку якщо в select вказано поля яких не існує у ресурсу то
повертати 400 код помилки.
...
В нашому випадку у відповіді з 202 статусом медіа тип **ПОВИНЕН** бути [application/vnd.rollun-long-task+json](#applicationvndrollun-long-task-pendingjson). 
...

{
  "task": {
    ...
    "resultId": "1" \\ ідентифікатор створеного ресурсу
  }
}
```
