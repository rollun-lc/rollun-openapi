...
Если вы не используете докер, то для тестов нужно чтобы приложение было доступно по адресу `localhost:8081`. Для этого можна
воспользоваться встроенным в php сервером запустив команду:

```bash
php -S localhost:8001 public/index.php
```

После чего тесты можно запустить через `composer test`
