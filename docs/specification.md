...
можемо створити ресурс-контролер /video/{id}/actions/convert. Процес конвертації запускається POST запитом з відомостями
про
...
| Код | Назва                 | Опис                                                                                                                                                                                                                           |
|-----|-----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 200 | OK                    | Під час виконання запиту не виникло будь-яких помилок (як бізнес логіки, так і серверу), що завадили б виконанню. Результат виконання присутній у відповіді. Може повернутись у відповіді для будь-якого http методу.          | 
| 201 | Created               | Виконання екшену завершено і результат присутній і відповіді. Може повернутись лише при виконані POST, або GET запиту до ресурсу екшену. Не потрібно використовувати цей код окремо від екшенів.                               |
| 202 | Accepted              | Запит принятий у опрацювання, але ще не виконано. У відповіді міститься ідентифікатор ресурсу, за допомогою якого можна відстежувати стан виконання запиту. Може повернутись у відповіді для будь-якого http методу, крім GET. |
| 403 | Forbidden             | Запит коректний, але під час його виконання відбулась помилка бізнес логіки, через яку запит не виконався. Повторний ідентичний запит не змінить ситуацію. Може повернутись у відповіді для будь-якого http методу.            |
| 500 | Internal Server Error | Під час виконання запиту відбулась помилка на сервері, через яку запит не виконався. Повторний ідентичний запит може успішно виконатись. Може повернутись у відповіді для будь-якого http методу.                              |
...
```GET /articles/actions/post/abc1``` запит може повернути коди: 200, 201, 403, 500. 

Код 403 і 500 повертається у ситуаціях аналогічних описаним вище. Просто вже як відповідь на GET, а не POST, запит.

Код 200 повертається, якщо виконання запиту ще в процесі. У тілі відповіді міститься інформація про стан виконання.
...
Код 201 повертається коли виконання запиту закінчилось успішно. Відповідь містить результат виконання, що у нашому
