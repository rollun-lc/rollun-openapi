...
В цьому форматі на мою думку є недоліки, головний з яких - те що ми придумаємо власний формат, хоча існує вже 
стандартизований в [RFC-7807](https://www.rfc-editor.org/rfc/rfc7807) формат з медіа типом `application/problem+json`.
Я пропоную розширювати його під наші потреби замість того щоб видумувати з нуля власний формат. 

Об'єкт опису помилки містить наступні поля:

- `title` (string) - human-readable повідомлення з коротким описом помилки.
...
```json
{
  "type": "urn:problem-type:rollun:validationError",
...
інтерфейсу (який згенерованний на [openapi рівні](#openapi-рівень)). Така схема дозволить реалізовувати логіку 
вибору сервера будь-якої складності.

![Host selection](./img/arch/host-selection.jpg)
