...
Обидві проблеми можна вирішити зробивши механізм наслідування маніфестів один від одного, але це суттєва зміна,
яка буде потребувати дописувати сторонні бібліотеки, що працюють з openapi (той самий swagger-ui), а також потребує 
визначення правил наслідування, та створення механізмів контролю, що вони дотримуються. На мій полгяд це досить
об'ємна та не тривіальна задача, тому я б спробував її уникнути й шукав розв'язок для кожної проблеми окремо.

Для розв'язання першої проблеми, можна зробити кастомний атрибут, який вказував би на те, які сервера є дзеркалами один 
одного, а які ні, та заодно назвати їх. Наприклад назвемо цей атрибут `x-server-mirrors`:
...
В коді можна генерувати під кожне ім'я з x-server-mirrors окремий інтерфейс, який буде однозначно відповідати набору 
